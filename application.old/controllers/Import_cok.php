...
class Import_cok extends CI_Controller {
		...
		if ($handle = opendir($this->path_dir)) {
			while (false !== ($this->excel_file = readdir($handle))) {
				if ($this->excel_file != "." && $this->excel_file != "..") {
					Import_cok::parse_xlsx($this->path_dir,$this->excel_file);
				}
			...
			foreach ($Spreadsheet as $Key => $Row)
			{
				if ($Row)
				{
					if($Row[0] != 'Agent')
					{
						...
						Import_cok::add_cok($agent_name, $date, $agent_login, $agent_logout, $all_call_count, $received_call_count, $missed_call_count);
					}
				}
			}
		}
